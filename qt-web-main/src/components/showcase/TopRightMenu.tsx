...
export default function TopRightMenu() {
    ...
    return (
        ...
        >
            <ul className="w-full flex px-8 pt-3">
                {btnDemo.map((item, index) => (
                    <li className="w-full px-2 box-border">
                        <button
                            key={index}
                            onClick={() => handleClick(index)}
                            className={`w-full justify-center py-2 rounded-tr-md rounded-bl-md leading-6 border rounded-tl-[20px] rounded-br-[20px] bg-[#e1e1e1] opacity-70 flex items-center ${
                                activeIndex === index
